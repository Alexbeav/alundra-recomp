...
#
# Writes: dist/Alundra-Recomp-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix Alundra-Recomp \
  --exe-name Alundra_Recompiled \
  --display-name "Alundra Recompiled" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --disc-hint "your legally owned Alundra disc" \
